package kr.co.ajsoft.ex39datastorageexternal;
...
public class MainActivity extends AppCompatActivity {
    ...
    public void clickSave(View view) {
        ...
        //여기까지 커서가 오면..
        //외장메모리가 연결되어있다고 볼수 있음.
        //외부메모리에 데이터 저장 작업 시작

        String data=et.getText().toString();
        et.setText("");

        File path; //파일 저장될 경로정보를 가진 객체[Data.txt파일이 저장될 경로]

        //Marshmallow(api 23버전) 이상에서는
        //SDcard의 아무 위치에나 직접 저장하는 것이
        //불가능하도록 보안을 강화함(동적퍼미션을 받아야 가능함)
        //오로지 각 앱에게 할당된 고유한 영역에만 저장이 가능함.
        //고유한 영역의 경로를 얻어오기
    ...
    public void clickLoad(View view) {
        ...
        if(state.equals(Environment.MEDIA_MOUNTED)||
           ...
           //맞으면 읽을 수 있는 상태
            ...
            File path=getExternalFilesDirs("Mydata")[0]; //메소드의 리턴값이 배열이라 []사용
    ...
    public void clickBtn(View view) {
        ...
        String state=Environment.getExternalStorageState();
        if(!state.equals(Environment.MEDIA_MOUNTED)){
            Toast.makeText(this, "외부저장소가 없음", Toast.LENGTH_SHORT).show();
            return;
        }

        //동적퍼미션 체크 작업(api23버전 이상일때)
